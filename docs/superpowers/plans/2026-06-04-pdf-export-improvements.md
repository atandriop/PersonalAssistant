...
**Goal:** Expand the PDF export from 4 sections to 16, extract the logic into `src/lib/exportPdf.ts`, and improve print CSS so multi-page tables render cleanly.

**Architecture:** A single async function `exportPdf()` is extracted from `SystemPage.tsx` into `src/lib/exportPdf.ts`. It fetches 18 API endpoints in parallel, generates a single HTML document, opens a print window, and calls `window.print()`. `SystemPage.tsx` imports and calls this function.

**Tech Stack:** Next.js 14, TypeScript, browser `window.print()` API
...
| Action | File |
|---|---|
| Create | `src/lib/exportPdf.ts` — complete export implementation |
| Modify | `src/components/system/SystemPage.tsx` — replace inline `exportPdf()` with import |
...
- [ ] **Step 1: Create `src/lib/exportPdf.ts` with the complete implementation**
...
git commit -m "feat: extract exportPdf to src/lib/exportPdf.ts with 16 sections"
```

---

## Task 2: Update SystemPage.tsx to use the new module

**Files:**
- Modify: `src/components/system/SystemPage.tsx`

- [ ] **Step 1: Add the import at the top of `src/components/system/SystemPage.tsx`**

After the existing imports (after `import useSWR from 'swr'`), add:

```typescript
import { exportPdf } from '@/lib/exportPdf'
```

- [ ] **Step 2: Remove the inline `exportPdf` function and update the state setter call**

Find the inline `async function exportPdf()` (around line 85, spans to line 194). Delete the entire function body.

Replace the button's `onClick` handler that calls `exportPdf()` — it already calls `exportPdf()` and `setExportingPdf`, so the button itself doesn't need to change. But the inline function called `setExportingPdf(true/false)` and used `try/finally`. Replace the deleted function with this wrapper that preserves the loading state:

Find where the export button calls `exportPdf()` in the JSX. Replace the entire deleted inline function with:
...
Then update the button's `onClick` from `exportPdf` to `handleExportPdf`.
...
git commit -m "feat: SystemPage uses extracted exportPdf from lib"
```
