# Finance Summary Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a read-only Finance Summary page showing net worth, portfolio breakdown by type, subscription costs, and wishlist value by priority.

**Architecture:** New `FinancePage` component fetches four existing endpoints, computes all aggregates client-side, and renders four read-only sections. No new API routes.
...
| Task | Files |
|------|-------|
| 1. FinancePage component | Create `src/components/finance/FinancePage.tsx` |
| 2. Route | Create `src/app/finance/page.tsx` |
| 3. Sidebar | Modify `src/components/Sidebar.tsx` |

---

## Task 1: Create FinancePage component

**Files:** Create `src/components/finance/FinancePage.tsx`
                ...
                {activeSubs.map(s => (
                  <div key={s.id} className="flex items-center justify-between py-1 border-b border-gray-50 dark:border-gray-800 last:border-0">
                    ...
                    <div className="text-right shrink-0 ml-2">
                      <span className="text-sm font-medium text-gray-900 dark:text-white">{fmtDecimal(s.monthly)}/mo</span>
                    </div>
                  </div>
                ))}
              </div>
...
git commit -m "feat: add FinancePage with net worth summary, portfolio breakdown, subscriptions, and wishlist"
```
...
**Files:** Create `src/app/finance/page.tsx`
...
git commit -m "feat: add /finance route"
```

---

## Task 3: Add Finance to sidebar

**Files:** Modify `src/components/Sidebar.tsx`

- [ ] **Step 3.1: Add Finance entry after Net Worth**
...
git commit -m "feat: add Finance to sidebar navigation"
```
